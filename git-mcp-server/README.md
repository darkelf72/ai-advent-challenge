...
- `GIT_MCP_SERVER_URL` - URL Git MCP сервера (по умолчанию: `http://localhost:8083`)
...
- Сервер работает с git-репозиторием в текущей директории (`.`)
- В Docker контейнере .git директория монтируется как read-only volume
